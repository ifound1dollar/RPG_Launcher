...
namespace RPG_Launcher.Model
{
    ...
    public class LoginApiService
    {
        ...
        public bool TryLoginFromRefreshToken(string refreshToken)
        {
            ...
            string[]? tokens = TempLoginApiImitator.LoginFromRefreshToken(refreshToken, AppStateData.ClientGuid);
        ...
        public bool Login(NetworkCredential credential)
        {
            ...
            string[]? tokens = TempLoginApiImitator.Login(credential.UserName, credential.Password, AppStateData.ClientGuid);
            if (tokens != null && tokens.Length == 2)
            {
                // Store saved username after successful login (must be done before refresh token is written).
                AppStateData.SavedUsername = credential.UserName;
        ...
        public bool Register(string email, NetworkCredential credential)
        {
            ...
            var tokens = TempLoginApiImitator.Register(email, credential.UserName, credential.Password, AppStateData.ClientGuid);
            if (tokens != null && tokens.Length == 2)
            {
                // Store saved username after successful login (must be done before refresh token is written).
                AppStateData.SavedUsername = credential.UserName;
